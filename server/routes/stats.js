...
function tenantMatch(req) {
  return req.tenantId ? { tenantId: req.tenantId } : {};
}

// Fast summary — only numbers
router.get('/summary-quick', async (req, res) => {
  try {
    const cacheKey = `summary:${req.tenantId || 'all'}:${JSON.stringify(req.query)}`;
...
router.get('/dashboard', async (req, res) => {
  try {
    const cacheKey = `dashboard:${req.tenantId || 'all'}:${JSON.stringify(req.query)}`;
    const cached   = getCached(cacheKey);
    if (cached) return res.json(cached);
    ...
    const [
      ...
      totalDueAgg, statusCountsAgg, todayDeliveries, outletsOverviewRaw,
      topItems, byPaymentMethod,
    ] = await Promise.all([
      Order.aggregate([
        { $match: { ...rangeBase, status: { $ne: 'Cancelled' } } },
        { $group: { _id: null, count: { $sum: 1 }, revenue: { $sum: '$payment.total' }, collected: { $sum: '$payment.advance' }, due: { $sum: '$payment.due' } } },
      ...
      Order.aggregate([
        { $match: { ...rangeBase, status: { $ne: 'Cancelled' } } },
        { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt', timezone: 'Asia/Kathmandu' } }, revenue: { $sum: '$payment.total' }, orders: { $sum: 1 } } },
        { $sort: { _id: 1 } },
      ...
      Order.find({
        ...
        isDeleted: { $ne: true },
      }).populate('outlet', 'name city').sort('delivery.slot').limit(100).lean(),
      Order.aggregate([
        { $match: { ...rangeBase, status: { $ne: 'Cancelled' } } },
        { $group: { _id: { outlet: '$outlet', status: '$status' }, count: { $sum: 1 }, revenue: { $sum: '$payment.total' }, due: { $sum: '$payment.due' } } },
        { $group: { _id: '$_id.outlet', totalOrders: { $sum: '$count' }, revenue: { $sum: '$revenue' }, due: { $sum: '$due' }, statuses: { $push: { status: '$_id.status', count: '$count' } } } },
      ...
      Order.aggregate([
        ...
        { $group: { _id: '$payment.method', collected: { $sum: '$payment.advance' }, total: { $sum: '$payment.total' }, count: { $sum: 1 } } },
        { $sort: { collected: -1 } },
      ]),
    ]);

    const outletFilter = req.tenantId ? { isActive: true, tenantId: req.tenantId } : { isActive: true };
    const allOutlets   = await Outlet.find(outletFilter, '_id name city').lean();
    ...
    outletsOverviewRaw.forEach((r) => {
      ...
      outletMap[id] = {
        ...
        totalOrders: r.totalOrders, revenue: r.revenue, due: r.due,
        statuses: Object.fromEntries(r.statuses.map((s) => [s.status, s.count])),
      };
    });
    allOutlets.forEach((o) => {
      if (!outletMap[o._id.toString()]) {
        outletMap[o._id.toString()] = { outlet: o, totalOrders: 0, revenue: 0, due: 0, statuses: {} };
      }
    });

    const todayActiveCounts = await Order.aggregate([
      { $match: { ...tm, 'delivery.date': { $gte: todayStart, $lte: todayEnd }, status: { $nin: ['Delivered','Cancelled'] }, isDeleted: { $ne: true } } },
    ...
    const result = {
      ...
      byStatus, byChannel, byCity, byPaymentMethod, revenueChart,
      todayDeliveries,
...
router.get('/summary', async (req, res) => {
  ...
  try {
    const [todayOrders, revenueAgg, totalDueAgg, pendingCount] = await Promise.all([
      Order.countDocuments({ ...tm, 'delivery.date': { $gte: start, $lte: end }, status: { $ne: 'Cancelled' }, isDeleted: { $ne: true } }),
      Order.aggregate([{ $match: { ...tm, createdAt: { $gte: start, $lte: end }, status: { $ne: 'Cancelled' }, isDeleted: { $ne: true } } }, { $group: { _id: null, total: { $sum: '$payment.total' } } }]),
      Order.aggregate([{ $match: { ...tm, status: { $nin: ['Delivered','Cancelled'] }, isDeleted: { $ne: true } } }, { $group: { _id: null, due: { $sum: '$payment.due' } } }]),
      Order.countDocuments({ ...tm, status: 'In Production', isDeleted: { $ne: true } }),
    ]);
...
router.get('/by-channel', async (req, res) => {
  const tm = tenantMatch(req);
  try {
    const data = await Order.aggregate([{ $match: { ...tm, status: { $ne: 'Cancelled' }, isDeleted: { $ne: true } } }, { $group: { _id: '$sender.channel', count: { $sum: 1 } } }, { $sort: { count: -1 } }]);
...
router.get('/by-city', async (req, res) => {
  const tm = tenantMatch(req);
  try {
    const data = await Order.aggregate([{ $match: { ...tm, status: { $ne: 'Cancelled' }, isDeleted: { $ne: true } } }, { $group: { _id: '$receiver.city', count: { $sum: 1 } } }, { $sort: { count: -1 } }]);
    res.json({ success: true, data });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
...
router.get('/by-status', async (req, res) => {
  const tm = tenantMatch(req);
  try {
    const data = await Order.aggregate([{ $match: { ...tm, isDeleted: { $ne: true } } }, { $group: { _id: '$status', count: { $sum: 1 } } }]);
    res.json({ success: true, data });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
...
router.get('/revenue-7days', async (req, res) => {
  ...
  try {
    ...
    const data = await Promise.all(days.map(async (day) => {
      ...
      const agg  = await Order.aggregate([{ $match: { ...tm, createdAt: { $gte: day, $lt: next }, status: { $ne: 'Cancelled' }, isDeleted: { $ne: true } } }, { $group: { _id: null, revenue: { $sum: '$payment.total' } } }]);
...
router.get('/on-this-day', async (req, res) => {
  ...
  try {
    ...
    const results = await Promise.all(years.map(async (year) => {
      ...
      const orders = await Order.find({ ...tm, 'delivery.date': { $gte: start, $lte: end }, status: { $ne: 'Cancelled' }, isDeleted: { $ne: true } })
        .select('orderNumber sender items payment status delivery').lean();
      return { year, orders };
    }));
    res.json({ success: true, data: results.filter((r) => r.orders.length > 0) });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
